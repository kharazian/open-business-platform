...
export function validateTriggerDraft(draft: TriggerDraft): TriggerDraftValidationResult {
  ...
  draft.actions.forEach((action, index) => {
    ...
    if (action.type === "call_webhook") {
      ...
      const webhookHeaders = parseWebhookHeaders(action.webhookHeadersText);

      if (!webhookHeaders) {
        errors.push(error(`${path}.webhookHeaders`, "trigger.action.webhook_headers", "Enter valid webhook headers JSON."));
      } else if (Object.keys(webhookHeaders).some((header) => header.toLowerCase() === "idempotency-key")) {
        errors.push(error(`${path}.webhookHeaders`, "trigger.action.webhook_idempotency_header_reserved", "Idempotency-Key is generated by the platform."));
      }
    }
