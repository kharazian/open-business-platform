...
test("Business Performance template creates an independent four-section draft", () => {
  let sequence = 0;
  const instantiate = () => instantiateDashboardTemplate(
    businessPerformanceSampleTemplate,
    { sources: { primary: { formId } } },
    { idGenerator: () => `id-${++sequence}`, now: () => "2026-08-21T12:00:00.000Z" }
  );
  ...
  assert.equal(first.dashboard.config.sections.length, 4);
  assert.equal(first.dashboard.config.widgets.length, 10);
  assert.equal(first.dashboard.config.filters.length, 4);
...
test("template instantiation validates source bindings and ids", () => {
  ...
  const malformed = instantiateDashboardTemplate(businessPerformanceSampleTemplate, { sources: { primary: { formId: "not-a-guid" } } });
  assert.equal(malformed.ok, false);
  if (!malformed.ok) assert.ok(malformed.errors.some((error) => error.code === "template.binding.form_id_invalid"));

  const unknown = instantiateDashboardTemplate(businessPerformanceSampleTemplate, { sources: { primary: { formId }, extra: { formId } } });
...
test("sample capability validation explains missing reportable fields", () => {
  const available = new Set(["amount", "event_date", "category", "region", "priority", "status", "created_at", "title"]);
  assert.deepEqual(validateTemplateFieldCapabilities(businessPerformanceSampleTemplate, available), []);
  const errors = validateTemplateFieldCapabilities(businessPerformanceSampleTemplate, new Set(["status", "created_at"]));
