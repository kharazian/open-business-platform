...
import { createDashboardAdapterWidget } from "../adapters";
...
export function DashboardAdapterSettingsEditor({ adapter, onChange, value }: { adapter: DashboardAdapterRegistration; onChange: (value: DashboardAdapterWidget) => void; value: DashboardAdapterWidget }) {
  ...
  return <div className="grid gap-4">
    <Select label="Visualization" onChange={(event) => { const next = createDashboardAdapterWidget(adapter, event.target.value); if (next) onChange(next); }} options={adapter.visualizations.map((item) => ({ label: item.name, value: item.id }))} value={visualization.id} />
