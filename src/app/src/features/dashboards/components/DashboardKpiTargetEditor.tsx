...
import type { DashboardKpiGoalDirection, DashboardKpiTarget } from "../types";

const directions: Array<{ label: string; value: DashboardKpiGoalDirection }> = [
...
export function DashboardKpiTargetEditor({ onChange, value }: { onChange: (value: DashboardKpiTarget) => void; value: DashboardKpiTarget }) {
  return <div className="grid gap-3 rounded-xl border border-border bg-card/60 p-3">
    ...
    {value.enabled ? <div className="grid gap-3 sm:grid-cols-2">
      ...
      <Select id="kpi-target-direction" label="Goal direction" onChange={(event) => onChange({ ...value, direction: event.target.value as DashboardKpiGoalDirection })} options={directions} value={value.direction} />
