...
import type { DashboardAdapterRegistration, DashboardAnalyticsFilterValue, DashboardAnalyticsResponse, DashboardAnalyticsWidgetType, DashboardCardAccent, DashboardChartAppearance, DashboardChartPalette, DashboardChartSeriesDefinition, DashboardInteractionDestination, DashboardNumberFormat, DashboardSeriesAxis, DashboardSeriesColor, DashboardSeriesDisplayType, DashboardWidgetWidth, SavedDashboardSection, SavedDashboardWidget, SavedDashboardWidgetLayout } from "../types";
...
export function DashboardWidgetPropertiesDrawer({ adapters, forms, layout, onApply, onClose, open, sections, widget }: {
  ...
  const fixedFilterFields = fields.filter((field) => field.filterable && field.options.length > 0);
  ...
  function addFixedFilter() { const used = new Set(activeDraft.chart?.fixedFilters?.map((filter) => filter.fieldId) ?? []); const field = fixedFilterFields.find((candidate) => !used.has(candidate.id)); if (field) updateChart({ fixedFilters: [...(activeDraft.chart?.fixedFilters ?? []), { fieldId: field.id, values: [field.options[0].value] }] }); }
      ...
      <div className="grid min-h-0 flex-1 gap-6 overflow-y-auto p-5">
        ...
        {draft.chart ? <section className="grid gap-4 border-t border-border pt-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <Select disabled={loadingSource} label="Source form" onChange={(event) => update({ sourceFormId: event.target.value, chart: { ...draft.chart!, reportId: null, fixedFilters: [] }, interaction: null })} value={draft.sourceFormId ?? ""}>{forms.map((form) => <option key={form.id} value={form.id}>{form.name}</option>)}</Select>
          ...
          <div className="grid gap-3 rounded-xl border border-border bg-muted/10 p-3"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-bold">Fixed widget filters</p><p className="text-xs text-muted-foreground">Always restrict this widget. Dashboard filters cannot replace a fixed value for the same field.</p></div><Button disabled={(draft.chart!.fixedFilters?.length ?? 0) >= 8 || fixedFilterFields.every((field) => draft.chart?.fixedFilters?.some((filter) => filter.fieldId === field.id))} onClick={addFixedFilter} size="sm" variant="outline"><Plus className="size-4" />Add filter</Button></div>{(draft.chart!.fixedFilters ?? []).length === 0 ? <p className="text-sm text-muted-foreground">No fixed filters. This widget uses all permitted source records.</p> : (draft.chart!.fixedFilters ?? []).map((filter, index) => { const selectedField = fixedFilterFields.find((field) => field.id === filter.fieldId); return <div className="grid gap-3 rounded-lg border border-border bg-card p-3 sm:grid-cols-[1fr_1fr_auto]" key={`${filter.fieldId}-${index}`}><Select label="Field" onChange={(event) => { const field = fixedFilterFields.find((candidate) => candidate.id === event.target.value)!; updateFixedFilter(index, { fieldId: field.id, values: [field.options[0].value] }); }} value={filter.fieldId}>{fixedFilterFields.map((field) => <option disabled={(draft.chart?.fixedFilters ?? []).some((item, current) => current !== index && item.fieldId === field.id)} key={field.id} value={field.id}>{field.label}</option>)}</Select><Select label="Required value" onChange={(event) => updateFixedFilter(index, { fieldId: filter.fieldId, values: [event.target.value] })} value={filter.values?.[0] ?? ""}>{(selectedField?.options ?? []).map((option) => <option key={option.id} value={option.value}>{option.label}</option>)}</Select><Button aria-label={`Remove fixed filter ${selectedField?.label ?? filter.fieldId}`} className="self-end" onClick={() => removeFixedFilter(index)} size="icon" variant="ghost"><Trash2 className="size-4" /></Button></div>; })}</div>
...
export function isDashboardAnalyticsWidgetDraftValid(widget: SavedDashboardWidget, fields: ReportableField[]) {
  ...
  const validFixedFilters = fixedFilters.length <= 8 && new Set(fixedFilterIds).size === fixedFilterIds.length && fixedFilters.every((filter) => {
    ...
    return Boolean(field?.filterable)
      && (values.length > 0 || Boolean(filter.start || filter.end))
      && (allowedValues.size === 0 || values.every((value) => allowedValues.has(value)));
  });
  return validSeries && validFixedFilters && (type !== "table" || series.length === 1) && (chart.metric.type === "count" || Boolean(chart.metric.fieldId && ids.has(chart.metric.fieldId))) && (type !== "breakdown" || Boolean(chart.groupByFieldId && ids.has(chart.groupByFieldId))) && (type !== "trend" || Boolean(chart.dateFieldId && ids.has(chart.dateFieldId))) && (type !== "table" || Boolean(chart.columns?.length && chart.columns.every((id) => ids.has(id))));
}
