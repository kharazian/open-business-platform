...
import type { DashboardAdapterRegistration, DashboardAnalyticsFilterValue, DashboardAnalyticsResponse, DashboardAnalyticsWidgetType, DashboardBarMode, DashboardBarOrientation, DashboardCardAccent, DashboardCategorySort, DashboardChartAppearance, DashboardChartAxes, DashboardChartPalette, DashboardChartSeriesDefinition, DashboardDataLabelContent, DashboardDataLabelPosition, DashboardDateGranularity, DashboardDisplayUnit, DashboardEmptyPeriodBehavior, DashboardInteractionDestination, DashboardKpiComparison, DashboardLegendPosition, DashboardNullValueBehavior, DashboardNumberFormat, DashboardSeriesAxis, DashboardSeriesColor, DashboardSeriesDisplayType, DashboardTooltipContent, DashboardWidgetWidth, SavedDashboardSection, SavedDashboardWidget, SavedDashboardWidgetLayout } from "../types";
...
const dateGranularities: Array<{ label: string; value: DashboardDateGranularity }> = [{ label: "Day", value: "day" }, { label: "Week", value: "week" }, { label: "Month", value: "month" }, { label: "Quarter", value: "quarter" }, { label: "Year", value: "year" }];
const emptyPeriodBehaviors: Array<{ label: string; value: DashboardEmptyPeriodBehavior }> = [{ label: "Omit empty periods", value: "omit" }, { label: "Show as zero", value: "zero" }, { label: "Show a gap", value: "gap" }];
const nullValueBehaviors: Array<{ label: string; value: DashboardNullValueBehavior }> = [{ label: "Ignore null values", value: "ignore" }, { label: "Treat null as zero", value: "zero" }];
...
const legendPositions: Array<{ label: string; value: DashboardLegendPosition }> = [{ label: "Top", value: "top" }, { label: "Bottom", value: "bottom" }, { label: "Left", value: "left" }, { label: "Right", value: "right" }];
const dataLabelContents: Array<{ label: string; value: DashboardDataLabelContent }> = [{ label: "Automatic", value: "auto" }, { label: "Value", value: "value" }, { label: "Percentage", value: "percentage" }, { label: "Value and percentage", value: "value_and_percentage" }];
const dataLabelPositions: Array<{ label: string; value: DashboardDataLabelPosition }> = [{ label: "Automatic", value: "auto" }, { label: "Inside", value: "inside" }, { label: "Outside", value: "outside" }];
...
const displayUnits: Array<{ label: string; value: DashboardDisplayUnit }> = [{ label: "None", value: "none" }, { label: "Automatic (K/M/B)", value: "auto" }, { label: "Thousands (K)", value: "thousands" }, { label: "Millions (M)", value: "millions" }, { label: "Billions (B)", value: "billions" }];
const tooltipContents: Array<{ label: string; value: DashboardTooltipContent }> = [{ label: "Automatic", value: "auto" }, { label: "Value only", value: "value" }, { label: "Category and value", value: "category_value" }, { label: "Series, category and value", value: "series_category_value" }];
...
export function DashboardWidgetPropertiesDrawer({ adapters, forms, layout, onApply, onClose, open, sections, widget }: {
  ...
  const canGroupRemainingCategories = effectiveSeries.every((series) => series.metric.type !== "average");
  const circularSeries = effectiveSeries.some((series) => isDashboardCircularDisplayType(series.displayType));
  const percentageDataLabels = circularSeries || appearance.barMode === "stacked_percent";
  ...
  function changeType(next: DashboardAnalyticsWidgetType) {
    ...
    const nextCategoryLimit = normalizeDashboardCategoryLimit(appearance.categoryLimit, appearance.groupRemainingCategories, nextWidgetType, nextSeries ?? []);
    const nextLegendPosition = isDashboardLegendPositionValid(appearance.legendPosition, nextWidgetType) ? appearance.legendPosition : "top";
    const nextDataLabels = normalizeDashboardDataLabelSettings(appearance.dataLabelContent, appearance.dataLabelPosition, nextWidgetType, nextSeries ?? [], nextBarMode);
    const nextAxes = normalizeDashboardChartAxes(appearance.axes, nextWidgetType, nextSeries ?? [], nextReferenceLines, nextBarMode);
    updateChart({ widgetType: nextWidgetType, groupByFieldId: next === "breakdown" ? (activeDraft.chart.groupByFieldId || groupFields[0]?.id || null) : null, dateFieldId: next === "trend" ? (activeDraft.chart.dateFieldId || dateFields[0]?.id || null) : null, dateGranularity: next === "trend" ? (activeDraft.chart.dateGranularity ?? "day") : "day", emptyPeriodBehavior: next === "trend" ? (activeDraft.chart.emptyPeriodBehavior ?? "omit") : "omit", columns: next === "table" ? (activeDraft.chart.columns?.length ? activeDraft.chart.columns : fields.slice(0, 5).map((field) => field.id)) : [], series: nextSeries, kpiComparison: next === "summary" ? activeDraft.chart.kpiComparison : activeDraft.chart.kpiComparison ? { ...activeDraft.chart.kpiComparison, enabled: false } : null, appearance: next === "summary" ? { ...appearance, referenceLines: [], barMode: nextBarMode, barOrientation: nextBarOrientation, categorySort: nextCategorySort, categoryLimit: nextCategoryLimit.limit, groupRemainingCategories: nextCategoryLimit.groupRemaining, legendPosition: nextLegendPosition, dataLabelContent: nextDataLabels.content, dataLabelPosition: nextDataLabels.position, axes: nextAxes } : { ...appearance, conditionalFormatting: { ...appearance.conditionalFormatting, enabled: false }, kpiTarget: { ...appearance.kpiTarget, enabled: false }, referenceLines: nextReferenceLines, barMode: nextBarMode, barOrientation: nextBarOrientation, categorySort: nextCategorySort, categoryLimit: nextCategoryLimit.limit, groupRemainingCategories: nextCategoryLimit.groupRemaining, legendPosition: nextLegendPosition, dataLabelContent: nextDataLabels.content, dataLabelPosition: nextDataLabels.position, axes: nextAxes } });
  }
  ...
  function setSeries(series: DashboardChartSeriesDefinition[]) {
    ...
    const nextDataLabels = normalizeDashboardDataLabelSettings(appearance.dataLabelContent, appearance.dataLabelPosition, activeDraft.chart!.widgetType, series, nextBarMode);
    const nextCategoryLimit = normalizeDashboardCategoryLimit(appearance.categoryLimit, appearance.groupRemainingCategories, activeDraft.chart!.widgetType, series);
    const nextAppearance = {
      ...
      dataLabelContent: nextDataLabels.content,
      dataLabelPosition: nextDataLabels.position,
      categoryLimit: nextCategoryLimit.limit,
      groupRemainingCategories: nextCategoryLimit.groupRemaining,
  ...
  function changeSeriesDisplay(index: number, displayType: DashboardSeriesDisplayType) {
    ...
    const nextDataLabels = normalizeDashboardDataLabelSettings(appearance.dataLabelContent, appearance.dataLabelPosition, activeDraft.chart!.widgetType, next, nextBarMode);
    ...
    updateChart({ series: next, metric: { ...next[0].metric }, appearance: { ...appearance, referenceLines: nextReferenceLines, barMode: nextBarMode, barOrientation: nextBarOrientation, dataLabelContent: nextDataLabels.content, dataLabelPosition: nextDataLabels.position, axes: nextAxes } });
  }
  function changeBarMode(barMode: DashboardBarMode) { const referenceLines = barMode === "stacked_percent" ? [] : appearance.referenceLines; const labels = normalizeDashboardDataLabelSettings(appearance.dataLabelContent, appearance.dataLabelPosition, activeDraft.chart!.widgetType, effectiveSeries, barMode); updateAppearance({ barMode, referenceLines, dataLabelContent: labels.content, dataLabelPosition: labels.position, axes: normalizeDashboardChartAxes(appearance.axes, activeDraft.chart!.widgetType, effectiveSeries, referenceLines, barMode) }); }
  ...
  function toggleCategoryLimit(enabled: boolean) { updateAppearance({ categoryLimit: enabled ? Math.min(4, activeDraft.chart?.limit ?? 10) : null, groupRemainingCategories: false }); }
  function changeLegendPosition(legendPosition: DashboardLegendPosition) { updateAppearance({ legendPosition }); }
      ...
      <div className="grid min-h-0 flex-1 gap-6 overflow-y-auto p-5">
        ...
        {draft.chart ? <section className="grid gap-4 border-t border-border pt-5">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            {analyticsType === "trend" ? <Select help="Calendar buckets use the workspace timezone; weeks use its configured first day." label="Trend granularity" onChange={(event) => updateChart({ dateGranularity: event.target.value as DashboardDateGranularity })} options={dateGranularities} value={draft.chart.dateGranularity ?? "day"} /> : null}
            {analyticsType === "trend" ? <Select help="Choose whether calendar periods without usable data disappear, render as zero, or remain visible as a chart gap." label="Empty periods" onChange={(event) => updateChart({ emptyPeriodBehavior: event.target.value as DashboardEmptyPeriodBehavior })} options={emptyPeriodBehaviors} value={draft.chart.emptyPeriodBehavior ?? "omit"} /> : null}
            <Select disabled={effectiveSeries.every((series) => series.metric.type === "count")} help={effectiveSeries.every((series) => series.metric.type === "count") ? "Record counts do not read numeric field values." : "Ignore averages only present numeric values; zero includes null numeric values in the denominator."} label="Null numeric values" onChange={(event) => updateChart({ nullValueBehavior: event.target.value as DashboardNullValueBehavior })} options={nullValueBehaviors} value={draft.chart.nullValueBehavior ?? "ignore"} />
            <Input help={analyticsType === "breakdown" ? "Maximum source categories returned. Top-N and Other work within this bounded result." : undefined} label="Result limit" max={50} min={1} onChange={(event) => updateChart({ limit: Math.max(1, Math.min(50, Number(event.target.value) || 1)) })} type="number" value={draft.chart.limit ?? 10} />
          </div>
          ...
          {analyticsType !== "table" ? <div className="grid gap-4 rounded-xl border border-border bg-muted/10 p-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <Select help="Scale large values for display only; analytics and tooltips retain the same underlying value." label="Display units" onChange={(event) => updateAppearance({ displayUnit: event.target.value as DashboardDisplayUnit })} options={displayUnits} value={appearance.displayUnit} />
              {appearance.numberFormat !== "auto" || appearance.displayUnit !== "none" ? <Input label="Decimal places" max={4} min={0} onChange={(event) => updateAppearance({ decimalPlaces: Math.max(0, Math.min(4, Number(event.target.value) || 0)) })} type="number" value={appearance.decimalPlaces} /> : null}
              <Input help="Optional text placed before every formatted value (maximum 12 characters)." label="Value prefix (optional)" maxLength={12} onChange={(event) => updateAppearance({ valuePrefix: event.target.value })} placeholder="e.g. ~ or #" value={appearance.valuePrefix} />
              <Input help="Optional unit placed after every formatted value (maximum 24 characters). Include a leading space when needed." label="Value suffix (optional)" maxLength={24} onChange={(event) => updateAppearance({ valueSuffix: event.target.value })} placeholder="e.g. kg or hours" value={appearance.valueSuffix} />
              ...
              {(analyticsType === "breakdown" || analyticsType === "trend") ? <Select help="Side legends stack above the chart on narrow screens." label="Legend position" onChange={(event) => changeLegendPosition(event.target.value as DashboardLegendPosition)} options={legendPositions} value={appearance.legendPosition} /> : null}
            ...
            <div className="grid gap-3 rounded-lg border border-border bg-card p-3 sm:grid-cols-2"><Checkbox checked={appearance.showTooltips} description="Show a native value hint when a pointer pauses over a chart mark." label="Show tooltips" onChange={(event) => updateAppearance({ showTooltips: event.target.checked })} />{appearance.showTooltips ? <Select help="Accessible interaction labels always retain full context." label="Tooltip content" onChange={(event) => updateAppearance({ tooltipContent: event.target.value as DashboardTooltipContent })} options={tooltipContents} value={appearance.tooltipContent} /> : <p className="self-center text-xs text-muted-foreground">Tooltips are hidden; keyboard and screen-reader interaction labels remain available.</p>}</div>
            {analyticsType === "breakdown" ? <div className="grid gap-3 rounded-lg border border-border bg-card p-3 sm:grid-cols-2"><Checkbox checked={appearance.categoryLimit !== null} description="Show a focused subset of returned categories." label="Limit visible categories" onChange={(event) => toggleCategoryLimit(event.target.checked)} />{appearance.categoryLimit !== null ? <Input help="Choose between 1 and 12 categories." label="Top categories" max={12} min={1} onChange={(event) => updateAppearance({ categoryLimit: Math.max(1, Math.min(12, Number(event.target.value) || 1)) })} type="number" value={appearance.categoryLimit} /> : null}<Checkbox checked={appearance.groupRemainingCategories} description={canGroupRemainingCategories ? "Sum the remaining returned groups into one aggregate." : "Other is unavailable because category averages cannot be summed accurately."} disabled={appearance.categoryLimit === null || !canGroupRemainingCategories} label="Group remaining as Other" onChange={(event) => updateAppearance({ groupRemainingCategories: event.target.checked })} /><p className="self-center text-xs text-muted-foreground">Other is an aggregate and will not create a misleading single-category drill-through filter.</p></div> : null}
            {appearance.showDataLabels && (analyticsType === "breakdown" || analyticsType === "trend") ? <div className="grid gap-3 rounded-lg border border-border bg-card p-3 sm:grid-cols-2"><Select help={percentageDataLabels ? "Choose values, percentages, or both." : "Percentage labels are available for pie, donut, and 100% stacked charts."} label="Data label content" onChange={(event) => updateAppearance({ dataLabelContent: event.target.value as DashboardDataLabelContent })} value={appearance.dataLabelContent}>{dataLabelContents.map((item) => <option disabled={(item.value === "percentage" || item.value === "value_and_percentage") && !percentageDataLabels} key={item.value} value={item.value}>{item.label}</option>)}</Select>{!circularSeries ? <Select help="Automatic keeps Bar labels readable and places line labels outside." label="Data label position" onChange={(event) => updateAppearance({ dataLabelPosition: event.target.value as DashboardDataLabelPosition })} options={dataLabelPositions} value={appearance.dataLabelPosition} /> : <p className="self-end text-xs text-muted-foreground">Pie and donut labels use the collision-safe label list.</p>}</div> : null}
          </div> : null}
...
export function isDashboardAnalyticsWidgetDraftValid(widget: SavedDashboardWidget, fields: ReportableField[]) {
  ...
  const dateGranularityValid = ["day", "week", "month", "quarter", "year"].includes(chart.dateGranularity ?? "day");
  const missingValuesValid = ["omit", "zero", "gap"].includes(chart.emptyPeriodBehavior ?? "omit") && ["ignore", "zero"].includes(chart.nullValueBehavior ?? "ignore");
  return validSeries && dateGranularityValid && missingValuesValid && isDashboardSeriesPresentationValid(chart.widgetType, series) && isDashboardReferenceLinesValid(appearance.referenceLines, chart.widgetType, series) && isDashboardBarModeValid(appearance.barMode, chart.widgetType, series, appearance.referenceLines) && isDashboardBarOrientationValid(appearance.barOrientation, chart.widgetType, series) && isDashboardCategorySortValid(appearance.categorySort, chart.widgetType) && isDashboardCategoryLimitValid(appearance.categoryLimit, appearance.groupRemainingCategories, chart.widgetType, series) && isDashboardLegendPositionValid(appearance.legendPosition, chart.widgetType) && isDashboardDataLabelSettingsValid(appearance.dataLabelContent, appearance.dataLabelPosition, chart.widgetType, series, appearance.barMode) && isDashboardChartAxesValid(appearance.axes, chart.widgetType, series, appearance.referenceLines, appearance.barMode) && isDashboardValueAffixValid(appearance.valuePrefix, appearance.valueSuffix) && isDashboardTooltipContentValid(appearance.tooltipContent) && validFixedFilters && comparisonValid && isDashboardConditionalFormattingValid(appearance.conditionalFormatting, chart.widgetType) && isDashboardKpiTargetValid(appearance.kpiTarget, chart.widgetType) && (type !== "table" || series.length === 1) && (chart.metric.type === "count" || Boolean(chart.metric.fieldId && ids.has(chart.metric.fieldId))) && (type !== "breakdown" || Boolean(chart.groupByFieldId && ids.has(chart.groupByFieldId))) && (type !== "trend" || Boolean(chart.dateFieldId && ids.has(chart.dateFieldId))) && (type !== "table" || Boolean(chart.columns?.length && chart.columns.every((id) => ids.has(id))));
}
