...
import { cloneDashboardChartAppearance, defaultDashboardChartAppearance, getDashboardAccentColor, getDashboardEffectiveCardAccent, isDashboardConditionalFormattingValid, resolveDashboardChartAppearance } from "../appearance";
...
export function isDashboardAnalyticsWidgetDraftValid(widget: SavedDashboardWidget, fields: ReportableField[]) {
  ...
  return validSeries && validFixedFilters && isDashboardConditionalFormattingValid(appearanceForValidation(chart.appearance), chart.widgetType) && (type !== "table" || series.length === 1) && (chart.metric.type === "count" || Boolean(chart.metric.fieldId && ids.has(chart.metric.fieldId))) && (type !== "breakdown" || Boolean(chart.groupByFieldId && ids.has(chart.groupByFieldId))) && (type !== "trend" || Boolean(chart.dateFieldId && ids.has(chart.dateFieldId))) && (type !== "table" || Boolean(chart.columns?.length && chart.columns.every((id) => ids.has(id))));
}
function appearanceForValidation(value?: DashboardChartAppearance | null) { return resolveDashboardChartAppearance(value).conditionalFormatting; }
