...
import { cloneDashboardChartAppearance, defaultDashboardChartAppearance, getDashboardAccentColor, getDashboardEffectiveCardAccent, isDashboardConditionalFormattingValid, isDashboardKpiTargetValid, isDashboardReferenceLinesValid, resolveDashboardChartAppearance } from "../appearance";
...
export function DashboardWidgetPropertiesDrawer({ adapters, forms, layout, onApply, onClose, open, sections, widget }: {
  ...
  function updateReferenceLines(referenceLines: DashboardChartAppearance["referenceLines"]) { updateAppearance({ referenceLines }); }
  ...
  function changeSeriesDisplay(index: number, displayType: DashboardSeriesDisplayType) { const next = effectiveSeries.map((series, current) => current === index ? { ...series, displayType } : series); updateChart({ series: next, metric: { ...next[0].metric }, appearance: isDashboardCircularDisplayType(displayType) ? { ...appearance, referenceLines: [] } : appearance }); }
    ...
    <aside aria-describedby="widget-properties-description" aria-label="Widget properties" aria-modal="true" className="absolute inset-y-0 right-0 flex w-full max-w-2xl flex-col border-l border-border bg-background shadow-2xl" role="dialog">
      ...
      <div className="grid min-h-0 flex-1 gap-6 overflow-y-auto p-5">
        ...
        {draft.chart ? <section className="grid gap-4 border-t border-border pt-5">
          ...
          {(analyticsType === "breakdown" || analyticsType === "trend") && !effectiveSeries.some((series) => isDashboardCircularDisplayType(series.displayType)) ? <DashboardReferenceLinesEditor onChange={updateReferenceLines} value={appearance.referenceLines} /> : null}
