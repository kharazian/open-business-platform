...
public sealed class OpenBusinessPlatformDbContext : DbContext
{
    ...
    private static void ConfigureDashboards(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<DashboardDefinition>(entity =>
        {
            ...
            entity.Property(dashboard => dashboard.Status).HasColumnName("status").HasMaxLength(20).HasDefaultValue("draft").IsRequired();
            entity.Property(dashboard => dashboard.Slug).HasColumnName("slug").HasMaxLength(100);
            entity.Property(dashboard => dashboard.ShowInNavigation).HasColumnName("show_in_navigation").HasDefaultValue(false);
            entity.Property(dashboard => dashboard.MenuLabel).HasColumnName("menu_label").HasMaxLength(100);
            entity.Property(dashboard => dashboard.MenuIcon).HasColumnName("menu_icon").HasMaxLength(50);
            entity.Property(dashboard => dashboard.MenuOrder).HasColumnName("menu_order").HasDefaultValue(0);
            entity.Property(dashboard => dashboard.ViewPermission).HasColumnName("view_permission").HasMaxLength(120);
            entity.Property(dashboard => dashboard.PublishedAt).HasColumnName("published_at");
