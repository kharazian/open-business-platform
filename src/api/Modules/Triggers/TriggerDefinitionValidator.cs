...
public static class TriggerDefinitionValidator
{
        ...
        List<TriggerValidationError> errors)
    {
        ...
        foreach (var header in action.WebhookHeaders ?? new Dictionary<string, string>())
        {
            ...
            if (TriggerWebhookIdempotency.IsReservedHeader(header.Key))
            {
                errors.Add(Error($"{path}.webhookHeaders", "trigger.action.webhook_idempotency_header_reserved", $"{TriggerWebhookIdempotency.HeaderName} is generated by the platform and cannot be overridden."));
            }
        }
    }
