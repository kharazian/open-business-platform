...
The V4 task 001 backend foundation stores trigger definitions per form, stores trigger execution logs, and exposes management APIs. V8 task 011 replaces best-effort post-commit record dispatch with a transactional outbox: record creation, edits, field changes, assignments, direct status changes, workflow starts/transitions, and approval-completed transitions save their event payload in the same transaction as the record and audit/history writes.

A hosted outbox worker polls every 10 seconds, claims up to 25 due messages atomically with a unique claim id, and reclaims abandoned processing messages after a five-minute lease. Infrastructure delivery failures retry up to five attempts with bounded exponential backoff from 30 seconds to 15 minutes, then remain as `dead_letter` rows for diagnosis. Once dispatch reaches a trigger, its action failure is represented in `trigger_logs` and handled by the existing per-trigger retry system; it does not cause the event envelope to be redelivered. Delivery is at-least-once across process/database failures, so external actions still require provider-level idempotency where duplicates are unacceptable.

V8 task 012 adds form-scoped delivery operations to the trigger workspace. Form managers can see status counts, oldest-pending time, health state, and dead-letter metadata without receiving event payload JSON. A dead letter can be atomically reset for replay once, and that action writes `trigger_event_outbox_replayed` to the audit log. A daily retention worker deletes at most 500 completed envelopes per run after 30 days; pending, processing, and dead-letter rows are never removed by retention.
...
The V4 task 004 recovery slice lets form managers manually retry failed trigger execution logs. A retry replays the saved event input, skips actions already recorded as completed on its source execution, and creates a new log linked to the failed source log.

The V4 task 009 reliability slice schedules failed trigger logs for automatic retry with a conservative three-attempt default. A hosted worker claims due logs, skips disabled triggers, resumes after actions already recorded as completed, and creates fresh retry logs linked to the failed source log. V4 task 010 adds per-trigger retry policy editing, so managers can disable retries or adjust max attempts and delay within supported bounds.
...
V8 task 013 makes outbound webhook attempts receiver-deduplicatable. The platform owns the `Idempotency-Key` header and derives an opaque stable key from the trigger, action, and original event identity. Retries and outbox redelivery of that same action/event reuse the key; a different action or event occurrence gets a different key. Default webhook bodies and safe action-result metadata include the same key. This supports receiver-side idempotency but does not itself guarantee exactly-once processing by third parties.

V8 task 014 adds payload-free automation monitoring. `/health` remains a simple API liveness check; `/health/automation` reports healthy/degraded delivery without failing liveness for backlog, and becomes unhealthy only when its database query fails. `/metrics` exports bounded aggregate Prometheus series for outbox states, retry backlog, oldest pending age, thresholds, and overall automation health. Production metrics require a configured bearer token. A monitor worker emits structured warnings once per observed dead letter and when the oldest pending message crosses the configured age, including form/message IDs but never payload or record values.
...
The scheduler atomically claims due enabled trigger definitions through a five-minute database lease and writes normal trigger logs with schedule metadata for due time, lock time, completion time, final status, run source, next run when available, and skip reason when persisted schedule metadata cannot be used. Form managers can also run an enabled scheduled trigger immediately through a protected manual-run endpoint; that path uses the same claim and execution behavior and records `runSource = "manual"`. V4/V8 scheduled triggers do not support record conditions or record-dependent actions.
