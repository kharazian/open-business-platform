...
- Store at most eight fixed filters inside each analytics widget. Choice filters use declared options; date filters use an optional inclusive start and exclusive end with ordered bounds. Validate them against that widget's permitted source schema, apply them on the backend after normal authorization and record scoping, and give them precedence over runtime dashboard filters on the same field. Fixed filters constrain one widget only and never grant access or introduce cross-form joins.
