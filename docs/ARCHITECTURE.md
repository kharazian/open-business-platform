...
- Keep KPI conditional formatting as ordered, bounded presentation rules over the already-returned primary summary value. Evaluate the same allowlisted operators in the browser, use the first matching semantic accent with the static accent as fallback, and reject enabled rules on non-KPI widgets. Rules never alter queries, stored records, or authorization.
