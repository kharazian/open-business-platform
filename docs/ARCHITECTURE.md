...
- Keep KPI targets as bounded presentation metadata over the already-returned primary summary value. Format target and variance with the widget's locale-aware number settings, use an absolute difference when a zero target makes percentage variance undefined, and reject enabled targets on non-KPI widgets. Targets never alter analytics execution or authorization.
