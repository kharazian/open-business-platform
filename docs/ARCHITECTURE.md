...
- Bound analytics widgets to four series. The backend validates every metric field against reportable and visible fields, then executes each series over the same already-authorized form/report/filter record set. Route configured breakdown/trend series through shared SVG presentation renderers; bar/line/area support one to four series, while pie/donut require one non-negative breakdown series because circular proportions are not meaningful for trends, mixed scales, or negative values. Keep KPI, table, empty, and legacy payloads on their specialized fallbacks. Series metadata controls presentation only; it cannot introduce formulas, SQL, or cross-form joins.
- Keep dashboard appearance as bounded presentation metadata inside saved widget JSON. Palette presets and format options are backend-validated, rendered with the effective user locale, and never participate in analytics execution or permission decisions. Display-unit scaling is limited to unscaled, locale-aware automatic compact, thousands, millions, or billions. Optional printable prefixes/suffixes are bounded to 12/24 characters and wrap only the final escaped text; neither mechanism changes sorting, geometry, thresholds, or raw analytics values. Active cartesian axes may have bounded titles and positive manual maxima; Bar baselines stay at zero, SVG geometry clamps to the chosen scale, and visible clipping warnings disclose values or references above it. Percentage stacks retain a fixed 100% scale. Breakdown category sorting operates only on the bounded returned point set; combined series totals drive value order, while trends retain chronological source order. Grouped vertical bars remain the default; category breakdowns containing only same-axis Bar series may render horizontally without transposing analytics execution. Normal and 100% stacks require two to four Bar series on one axis, use cumulative/percentage scales respectively, and reject negative source values in the renderer. Percentage stacks preserve actual values in tooltips and reject absolute reference lines. Cartesian charts may contain at most four labeled, nonnegative reference lines with semantic colors, bounded styles, and explicit axes; include their values in the matching display scale, but never treat them as analytics expressions or source-data targets.
- Keep chart legend position as bounded presentation metadata with top as the compatibility default. Top/bottom/left/right placement shares one renderer across cartesian and circular charts; side positions stack above the plot at narrow widths and long labels remain available as hover text.
- Keep data-label content and placement as bounded presentation metadata with automatic compatibility defaults. Derive percentages only from circular proportions or 100% stack totals already returned to the renderer. Cartesian labels may render automatically, inside, or outside marks; circular labels stay in the collision-safe label list.
- Keep native tooltip visibility and content as bounded presentation metadata with visible automatic compatibility defaults. Generate escaped text only from the already-returned category, series, formatted value, and derived percentage. Hiding a tooltip must not remove the complete interaction label used for keyboard and screen-reader access. Line and area marks retain bounded point targets without introducing another chart dependency.
- Keep Top-N category reduction in the shared presentation layer over the permission-filtered, API-bounded response. Order multi-series categories by combined returned values, aggregate each additive count/sum series' remainder independently, and keep Other last; average series may be limited but cannot enable Other because averages are not additive. Mark Other selections as aggregate so scalar drill-through cannot misrepresent several source categories as one stored value.
- Keep KPI conditional formatting as ordered, bounded presentation rules over the already-returned primary summary value. Evaluate the same allowlisted operators in the browser, use the first matching semantic accent and readable status label with the static accent as fallback, and reject enabled rules on non-KPI widgets. Required labels ensure that rule meaning is not communicated by color alone. Rules never alter queries, stored records, or authorization.
- Keep KPI targets as bounded presentation metadata over the already-returned primary summary value. Format target and variance with the widget's locale-aware number settings, use an absolute difference when a zero target makes percentage variance undefined, and evaluate favorable state through an allowlisted higher-is-better/lower-is-better direction. Bound progress to 0-100 for nonnegative values and omit it where a ratio would mislead. Reject enabled targets on non-KPI widgets. Targets never alter analytics execution or authorization.
- Execute KPI period comparisons on the backend over the same permission-scoped record set, saved-report constraints, hidden-field policy, and non-date filters as the primary aggregate. Use bounded UTC 7/30/90-day windows, replace filters only on the configured comparison date field, and return aggregates rather than source rows. The current-window result becomes the KPI value so the displayed actual and comparison share one scope.
...
- Store at most eight fixed filters inside each analytics widget. Choice filters use one or more declared options with OR semantics inside the field; different fields combine with AND semantics. Date filters use an optional inclusive start and exclusive end with ordered bounds. Validate them against that widget's permitted source schema, apply them on the backend after normal authorization and record scoping, and give them precedence over runtime dashboard filters on the same field. Fixed filters constrain one widget only and never grant access or introduce cross-form joins.
