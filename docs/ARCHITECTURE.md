...
- Store at most eight fixed filters inside each analytics widget. Validate them against that widget's permitted source schema, apply them on the backend after normal authorization and record scoping, and give them precedence over runtime dashboard filters on the same field. Fixed filters constrain one widget only and never grant access or introduce cross-form joins.
