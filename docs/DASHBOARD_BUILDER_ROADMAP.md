...
3. **Multi-series analytics contract — implemented**
   ...
   - Bar, line, area, and combo rendering with independent left/right scales; compatible category Bar series support vertical/horizontal direction plus grouped, stacked, and 100% stacked layouts; single category-breakdown series also support pie and donut presentation, without arbitrary formulas or SQL
4. **Appearance and color controls — implemented**
   - Theme-safe palette presets, per-series semantic colors, responsive top/bottom/left/right legends, bounded value/percentage data-label content and cartesian placement, configurable escaped native tooltips, gridlines, card accent, number/currency/percent formatting with automatic or fixed K/M/B display units and printable value affixes, deterministic breakdown category ordering, Top-N plus returned-category Other grouping, and bounded axis titles/manual maxima with clipping disclosure
