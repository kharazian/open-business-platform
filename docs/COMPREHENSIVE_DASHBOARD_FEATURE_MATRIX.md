...
| KPI conditional colors | Up to five ordered numeric rules with semantic accents and a static fallback | Verified | First-match evaluation is shared by editor preview and published viewer; backend rejects invalid operators, colors, bounds, duplicate IDs, excessive rules, and non-KPI use. |
