...
| Fixed widget filters | Per-widget choice and date-range constraints with saved-config precedence over same-field dashboard filters | Verified | Builder authoring, ordered inclusive-start/exclusive-end dates, backend schema/option validation, permission-scoped execution, cloning, and Operations module scoping are covered by focused tests. |
