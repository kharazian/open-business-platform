...
| Configured series charts | Add, remove, reorder, label, style, color, axis, and Bar layout for one to four metrics | Verified | Single and multi-series breakdown/trend charts honor bar/line/area display metadata; compatible Bar series support grouped, cumulative stacked, and percentage-stacked layouts; permission-filtered single-series breakdowns also support accessible, interactive pie/donut proportions, while combo charts use independent left/right scales. |
