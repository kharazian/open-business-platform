...
| KPI targets and variance | Optional labeled target with localized target and above/below variance display | Verified | Nonzero targets use percentage variance, zero targets use an absolute difference, and backend validation restricts bounded targets to KPI widgets. |
