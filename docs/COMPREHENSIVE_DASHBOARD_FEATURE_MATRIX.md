...
| Fixed widget filters | Per-widget choice constraints with saved-config precedence over same-field dashboard filters | Verified | Builder authoring, backend schema/option validation, permission-scoped execution, cloning, and Operations module scoping are covered by focused tests. |
