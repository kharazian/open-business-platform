...
The Playwright dashboard lifecycle suite starts or reuses the real Vite/API hosts and uses the local PostgreSQL development database. It creates a uniquely named restricted-audience one-widget dashboard, verifies directory search, previews and saves it in Chromium, publishes it, proves later draft edits are absent from slug/list/ID viewer responses, checks allowed and denied role audiences plus manager-only sharing/revision/recycle-bin metadata, republishes, restores an older revision without changing live, unpublishes, duplicates and archives an independent draft, restores it as a safe draft through the recycle-bin UI, re-archives it, verifies exact-name confirmation, and permanently deletes it. Its `finally` cleanup and pre-run sweep permanently remove active or archived `E2E dashboard ...` definitions. Run `npx playwright install chromium` once on a new machine.
