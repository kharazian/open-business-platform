...
The Playwright dashboard lifecycle suite starts or reuses the real Vite/API hosts and uses the local PostgreSQL development database. It creates a uniquely named restricted-audience one-widget dashboard, verifies directory search, previews and saves it in Chromium, publishes it, proves later draft edits are absent from slug/list/ID viewer responses, checks allowed and denied role audiences plus manager-only sharing/revision metadata, republishes, restores an older revision without changing live, unpublishes, duplicates an independent draft, archives that duplicate through the UI, and soft-deletes the original in `finally`. It also removes interrupted `E2E dashboard ...` runs before starting. Run `npx playwright install chromium` once on a new machine.
