...
- `src/app/src/features/triggers`: current V4 trigger management API client, builder helpers, form-scoped trigger workspace, logs viewer, and failed-log retry UI
...
- `src/api/Modules/Triggers`: current V4 backend trigger definitions, validation, detail/list/create/update APIs, event dispatch, starter action execution, trigger logs, and manual failed-log retry recovery
...
Next concrete work should continue with richer trigger actions, automatic retry policies, or the next approved product slice.
