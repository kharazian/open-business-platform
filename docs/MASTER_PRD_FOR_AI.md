...
- `src/app/src/features/dashboards`: dashboard summary/analytics clients, published dashboard directory and read-only slug viewer, permission-gated builder with user-friendly user/role/group audience sharing and searchable recycle bin, keyboard/touch canvas reordering and bounded preview concurrency, dynamic navigation, and generic adapter registry/settings contract
...
- `src/api/Modules/Identity`: V9 workspace-owned access policy management/evaluation for deny-overrides role, membership-role, department, group, record-status, and ownership guardrails
...
- `src/api/Modules/Triggers`: current V4 backend trigger definitions, validation, detail/list/create/update APIs, transactional record-event outbox/worker, payload-free automation health/metrics/monitoring, event dispatch, starter action execution, trigger logs, manual failed-log retry recovery, automatic retry queue worker, in-app notification trigger action, idempotency-keyed webhook action, user-authored retry policy handling, and scheduled trigger worker with explicit daily/weekly/monthly schedule contracts
...
- `src/api/Modules/Dashboards`: saved dashboard definition, isolated live publication snapshots for every normal-user read path, user/role/group audience enforcement, manager-only sharing and recycle-bin APIs, bounded revision history/restore, audited archive/safe-draft restore/retention-gated permanent deletion, publication/navigation, unique draft and published slugs, permission-filtered viewer/navigation APIs, generic adapter config validation, and audit logging
...
- `src/api/Modules/CreatorAnalysis`: V10 bounded memory-only untrusted-text analysis with credential suppression, typed sanitized findings, aggregate-only auditing, and no execution/import path
- `src/app/src/features/integrations`: integrations operations UI for API key lifecycle actions, connector configuration metadata, webhook listener operations, imports/exports, sanitized integration logs, bounded processing jobs, failure policies, processing health/operational logs, and analysis-only Creator export review
- `src/app/src/features/creator-analysis`: request-scoped Creator export submission plus sanitized typed report rendering and client-side JSON download with no import/apply control
- `src/api/Infrastructure/Persistence`: EF Core/Npgsql DbContext and migrations for users, identity/workspace policy, forms/versions/records, audit, reports/dashboards, triggers/workflows, printing, notifications, integrations, autonumber sequences, protected attachments, canonical record relationships, and bounded processing definitions/runs/operational logs
...
Advanced external notification delivery, report/scheduled PDF attachments, and custom code remain later modules. The Settings page keeps browser-local appearance separate from backend-owned workspace branding/localization and per-user localization overrides. Build product modules through the task files under `tasks/`.
...
V8 task 010 is complete: EF-backed aggregate concurrency stamps now prevent stale writes, scheduled triggers use atomic five-minute database claims, trigger retries preserve completed-action checkpoints, role-permission updates participate in parent-role concurrency, and export artifact bodies are available only through the audited download endpoint.
V8 task 011 is complete: record and workflow mutations now persist trigger event envelopes in the same PostgreSQL transaction, and a hosted worker uses atomic fenced claims, a five-minute abandoned-claim lease, bounded exponential retries, and dead-letter retention before invoking the existing trigger-log/action-retry layer.
V8 task 012 is complete: form managers now have payload-safe event-delivery health and dead-letter visibility in the trigger workspace, dead-letter replay is atomic and audited, and completed event envelopes are removed after 30 days in bounded daily batches.
V8 task 013 is complete: outbound trigger webhooks now carry a deterministic platform-owned `Idempotency-Key` that remains stable across action retries and transactional-outbox redelivery, and the same opaque key is available in default payloads and safe action results for receiver-side deduplication.
V8 task 014 is complete: automation delivery now has separate degraded health, configurable pending/dead-letter thresholds, protected aggregate Prometheus metrics, and payload-free structured warning logs without making backlog fail API liveness.
V8 finalization is documented in `docs/V8_FINALIZATION.md`, and practical V8 validation is documented in `docs/V8_PRACTICAL_TESTING.md`. V9 is finalized in `docs/V9_FINALIZATION.md`. V10 is finalized in `docs/V10_FINALIZATION.md`.
V9 task 001 is complete: tenant/workspace entities, stable default ownership, migration backfill, direct workspace foreign keys, centralized query filters/write guards, a single-default-workspace invariant, and an authenticated current-workspace endpoint establish the enterprise ownership boundary while preserving single-workspace behavior.
V9 task 002 is complete: workspace memberships, signed active context, per-request membership validation, workspace switching, scoped directory access, and workspace-aware integration identities preserve the tenant boundary across authenticated requests.
