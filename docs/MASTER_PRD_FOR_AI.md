...
- `src/app/src/features/triggers`: current V4 trigger management API client, builder helpers, form-scoped trigger workspace, and logs viewer
...
- `src/api/Modules/Triggers`: current V4 backend trigger definitions, validation, detail/list/create/update APIs, event dispatch, starter action execution, and trigger logs
...
Next concrete work should continue with richer trigger actions/retry behavior or the next approved product slice.
