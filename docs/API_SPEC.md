...
Analytics widget configs may additionally persist up to eight `fixedFilters` using the same field/value/date shape. Fixed filters are validated against the widget source schema, reject duplicate fields and undeclared choice values, and are always applied by the backend analytics engine. When a runtime dashboard filter targets the same field, the saved widget value takes precedence; other compatible runtime filters are combined normally. Changing a widget's source clears its fixed filters in the builder.
