...
Requires an authenticated application session. Returns the active tenant/workspace context resolved by the backend:

```json
{
  "tenantId": "00000000-0000-0000-0000-000000000001",
  "tenantName": "Open Business Platform",
...
Chart legend placement is an optional bounded appearance value: `top` (the backward-compatible default), `bottom`, `left`, or `right`. Breakdown and trend renderers apply it consistently to cartesian and circular charts; side legends stack above the chart on narrow screens. Non-chart widgets accept only the default position.

Data labels support bounded `auto`, `value`, `percentage`, or `value_and_percentage` content and `auto`, `inside`, or `outside` placement. Percentage content is accepted only for pie, donut, and `stacked_percent` charts. Cartesian charts support explicit placement; circular charts retain the collision-safe label list and accept only automatic placement. Missing settings use automatic content and placement, preserving the earlier renderer behavior.

Native value tooltips may be shown or hidden and support bounded `auto`, `value`, `category_value`, or `series_category_value` content. Automatic mode preserves contextual legacy wording. Percentage presentations retain the derived percentage beside the formatted raw value. Tooltip visibility never removes the visible value or the complete keyboard/screen-reader interaction label, and tooltip content cannot request arbitrary fields or markup.

Breakdown appearance may include an optional `categoryLimit` from 1 through 12 and `groupRemainingCategories`. The renderer selects Top-N from the already-returned bounded categories using the configured category order and combined multi-series values, then sums each additive count/sum series' remaining returned values into a final `Other` point when requested. Average series may use Top-N but reject Other grouping because category averages cannot be summed accurately. `Other` is aggregate metadata and is not converted into a scalar drill-through filter. Missing settings preserve the prior 12-category display cap without an Other point.

Analytics widget chart config may include an optional `appearance` object. Supported values are bounded to `theme`, `cool`, `warm`, or `mono` palettes; semantic card accents; boolean legend/data-label/gridline/tooltip flags; bounded tooltip content; `auto`, `number`, `currency`, or `percent` number formats; `none`, locale-aware `auto`, `thousands`, `millions`, or `billions` display units; an optional printable value prefix up to 12 characters and suffix up to 24 characters; a three-letter currency code; zero to four decimal places; `grouped`, `stacked`, or `stacked_percent` bar layout; `vertical` or `horizontal` bar orientation; `source`, value ascending/descending, or label ascending/descending category order; optional left/active-right axis titles up to 80 characters and positive bounded manual maxima; the bounded KPI-only presentation settings described below; and up to four reference lines for cartesian charts. Display units and value affixes affect rendered text only and reuse the shared formatter for KPI values, targets, comparisons, axes, labels, legends, and tooltips. Manual maxima are presentation-only, retain the zero baseline, clamp rendered geometry, and produce an explicit clipping warning when returned values or reference lines exceed the configured scale. A 100% stack always uses its fixed 100% maximum. Inactive axes and KPI, table, pie, and donut widgets reject non-default axis settings. Category ordering is restricted to breakdown charts and reorders only the already-returned bounded categories; multi-series value ordering uses each category's combined returned total. Trends preserve chronological source order. Stacking is restricted to breakdown/trend charts with two to four Bar series on the same axis. Horizontal orientation is restricted to category breakdowns containing only Bar series on one shared axis and reuses grouped or stacked layout behavior. A 100% stack rejects reference lines, uses a 0-100 display scale, and preserves actual values alongside percentages in tooltips. Each reference line has a unique bounded ID, required 1-80 character label, nonnegative bounded numeric value, semantic color, `solid`/`dashed`/`dotted` style, and `left`/`right` axis. KPI, table, pie, and donut widgets reject reference lines. Appearance is presentation-only and is not accepted as an analytics expression or database query input. Missing appearance config uses theme-aware legacy defaults, including visible automatic tooltips, empty affixes, and unscaled, source-ordered grouped vertical bars with automatic axes.
...
Analytics widget configs may additionally persist up to eight `fixedFilters` using the same field/value/date shape. A choice filter accepts up to 20 declared values and matches a record when any selected value matches; separate fixed-filter fields combine with AND semantics. Fixed filters are validated against the widget source schema, reject duplicate fields, undeclared choice values, value-based date filters, and date ranges whose exclusive end is not after the inclusive start. They are always applied by the backend analytics engine. When a runtime dashboard filter targets the same field, the saved widget value takes precedence; other compatible runtime filters are combined normally. Changing a widget's source clears its fixed filters in the builder.
...
KPI (`number_card`) appearance may include optional conditional formatting with at most five ordered rules. Each enabled rule has a bounded unique ID, one numeric threshold, an allowlisted comparison operator (`greater_than`, `greater_or_equal`, `less_than`, `less_or_equal`, or `equal`), a semantic accent, and a required 1-80 character status label. The first matching rule controls both the visible color and status text; the saved static card accent is the fallback when no rule matches. KPI appearance may also include one optional bounded numeric target with a required 1-80 character label and an allowlisted `higher_is_better` or `lower_is_better` goal direction. The viewer formats the target with the KPI number settings, derives a percentage above/below variance for nonzero targets or an absolute difference for a zero target, and presents direction-aware favorable/on-target/needs-attention text. Nonnegative actuals and targets also receive bounded 0-100 progress; negative-value targets omit progress rather than showing a misleading ratio. Conditional rules and targets do not modify analytics queries, values, permissions, or non-KPI widgets.

KPI chart config may additionally enable one permission-scoped period comparison using a reportable date/datetime field and `last_7_days`, `last_30_days`, or `last_90_days`. The server applies a UTC current window ending after the current date and the immediately preceding equal window, replacing other request filters on the selected comparison field while retaining every other fixed, runtime, saved-report, record-scope, and field-permission restriction. The primary KPI value becomes the current-window aggregate. The response may include only aggregate current/previous values, nullable percentage change when the previous value is zero, direction, and a bounded period label; it never exposes comparison records.
